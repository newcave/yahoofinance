...
@st.cache_data(ttl=7200)   # 2시간 캐시 → 하루 최대 ~6콜 (일한도 100 여유)
def load_smp(days: int = 3) -> tuple[pd.Series, pd.Series]:
    """SMP 육지/제주 — 공공데이터포털 공식 API (시간별 → 일평균)

    - 당일(delta=0) 제외: 당일 데이터는 API 미제공이 많아 불필요한 호출 낭비
    - 403 수신 즉시 루프 중단: 일일 한도(100콜) 초과 대응
    - serviceKey: quote()로 +→%2B 정확히 인코딩 후 URL 직접 삽입
    """
    try:
    ...
    for delta in range(1, days + 1):   # 1부터 시작 → 당일 제외
        if quota_exceeded:
            break
        ...
        for area_cd, bucket in [("1", land_rec), ("9", jeju_rec)]:
            ...
            try:
                ...
                if resp.status_code == 403:
                    st.warning("SMP API 일일 호출 한도(100건) 초과 — 내일 자정 이후 초기화됩니다.")
                    quota_exceeded = True
                    break

                if not resp.text.strip().startswith("{"):
                    st.warning(f"SMP API 비정상 응답 ({date_str}): {resp.text[:150]}")
                    continue
